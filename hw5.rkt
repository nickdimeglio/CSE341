        ...
        [(ifgreater? e)
         (let ([v1 (eval-under-env (ifgreater-e1 e) env)]
           ...
           (if (and (int? v1) (int? v2))
                ...
                (error "MUPL numerical comparison applied to non-number")))]        
        [(call? e) (let ([v1 (eval-under-env (call-funexp e) env)]
                         [v2 (eval-under-env (call-actual e) env)])
                     (if (closure? v1)
                         (letrec ([newenv (append
                                           (if (fun-nameopt (closure-fun v1))
                                               (list (cons (fun-formal (closure-fun v1))
                                                           v2)
                                                     (cons (fun-nameopt (closure-fun v1))
                                                           v1))
